...
function add(){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '${APP_PATH}/houtai/book/add'  //iframe的url
	});
}
...
function edit(id){
	...
	layer.open({
	  ...
	  shade: 0.8,
	  area: [w+'px', h+'px'],
	  content: '${APP_PATH}/houtai/book/edit?id='+id //iframe的url
	});
}
...
function del(ids){
	...
	$.post('${APP_PATH}/admin/book/delete',{ids:ids},function(result){
		if(result.success){
			layer.closeAll();
			layer.msg('删除成功');
			reload_data();
